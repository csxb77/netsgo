...
describe("TunnelTable client detail ownership projection", () => {
  ...
  test("uses the tunnel owner as the action client id", () => {
    expect(
      resolveTunnelOwnerClientId(
        createTunnel({ owner_client_id: "service-client" }),
        "detail-client",
      ),
    ).toBe("service-client");
    expect(
      resolveTunnelOwnerClientId(
        createTunnel({ client_id: "stored-client" }),
        "detail-client",
      ),
    ).toBe("stored-client");
  });
  ...
  test("filters fallback client proxy cache to owned tunnels only", () => {
    const source = getClientOwnedTunnelSource(
      undefined,
      [
        createTunnel({
          id: "owned",
          owner_client_id: "detail-client",
          client_id: "detail-client",
        }),
        createTunnel({
          id: "related",
          owner_client_id: "service-client",
  ...
  test("selects only tunnels eligible for each batch action", () => {
    const running = createTunnel({ id: "running" });
    const stopped = createTunnel({
      id: "stopped",
    ...
    expect(getBatchTunnelIds([running, stopped], "resume")).toEqual([
      "stopped",
    ]);
    expect(getBatchTunnelIds([running, stopped], "stop")).toEqual(["running"]);
  });
});
