...
By default, E2E probes run from inside the Compose network with `E2E_PROBE_MODE=internal`. This keeps reverse-proxy, server-ingress, and client-ingress assertions independent of Docker Desktop/OrbStack host-port forwarding. Set `E2E_PROBE_MODE=host` only when the test specifically needs to exercise published host ports.
...
The `v0.1.8` stable-only baseline was verified on historical commit `06aebb7` with:

```bash
make test-baseline-e2e COMPAT_BASELINE=v0.1.8 BASELINE_MODE=full
```

That run reused the local `netsgo-e2e:v0.1.8` image and passed `go test -tags=e2e ./test/e2e -run 'TestSystem.*E2E'` using only the stable image. The baseline explicitly skips current-only managed-client key rotation and the 1 MiB raw upload assertion because those contracts are not present in `v0.1.8`; current-version System E2E still covers both. It proves the stable baseline runtime, not the stable-image rebuild path. Run with `BASELINE_REBUILD_IMAGE=true` when the tag-to-image build path also needs proof.
...
The historical `06aebb7` full compat gate passed `11/11` with `make test-compat-e2e COMPAT_BASELINE=v0.1.8 COMPAT_MODE=full COMPAT_ABORT_ON_FAILURE=false`. That run used internal probes and the same two explicit old-version compatibility skips described above. An earlier run rebuilt `netsgo-e2e:v0.1.8` from tag `v0.1.8`, so it also exercised the stable-image rebuild path. The same historical worktree passed the nginx and caddy system gates with `E2E_PROBE_MODE=internal`.

`test-upgrade-e2e` starts from a stable running stack, creates real tunnels, verifies data paths, replaces server/client images, and revalidates the same tunnels. It includes server-only, target-client-only, ingress-client-only, clients-only, server rollback, current-write rollback, all-components upgrade, client-first rolling upgrade, and full cold upgrade cases. Server-only and target-only cover server-expose HTTP/TCP/UDP/SOCKS5. The full rolling/cold/current-write-rollback cases cover server-expose HTTP/TCP/UDP/SOCKS5 and client-to-client TCP/UDP/SOCKS5. The clients-only case keeps the stable server running after both clients are upgraded and then creates a new HTTP/TCP/UDP/SOCKS5 server-expose suite against the current target client. The rollback cases also create a new HTTP/TCP/UDP/SOCKS5 server-expose suite after returning to the stable server, then verify active state, empty issues, data paths, and listener counts on dedicated server alt ports. The narrower ingress-only case covers client-to-client TCP/SOCKS5. The harness also asserts empty tunnel issues plus listener counts for server-expose TCP/UDP/SOCKS5. The 1MiB TCP upload regression is covered by current-version `TestSystemE2E`; compatibility rows explicitly skip it when the selected old baseline does not provide the same post-reconnect contract. Upgrade remains a reconnect, persistence, rollback, and short data-path gate. The upgrade baseline uses the stable image for NetsGo-based helper services so the pre-upgrade stack is not coupled to the current image. The default reconnect/re-provision recovery window is `UPGRADE_RECOVERY_TIMEOUT_SECONDS=120`, forwarded by `make test-upgrade-e2e`; use the same variable when intentionally changing the upgrade wait budget.

The historical `06aebb7` upgrade gate passed `7/7 executed` with `E2E_PROBE_MODE=internal UPGRADE_ROLLBACK_MODE=auto make test-upgrade-e2e COMPAT_BASELINE=v0.1.8`; `2/9 planned` rollback cases were skipped because the `v0.1.8` server cannot read strict migration `009`. Use `UPGRADE_ROLLBACK_MODE=always` to intentionally exercise that fail-closed negative path. TCP-tunnel HTTP backend checks use `curl` against the exposed port instead of `nc`, because BSD/netcat timeout behavior can report false negatives even when the backend returned a valid HTTP response.

[KNOWN] After integrating remote commit `c00cfe6`, the current worktree passed the headless Playwright full suite `7/7` on 2026-08-12. The baseline, compatibility, and upgrade Docker gates have not been rerun; before release, rerun those three gates in CI or a clean Docker environment.
...
The historical `06aebb7` run also passed `make test-system-e2e-capability-loss` and `make test-p2p-double-nat-e2e`. The former covers capability-loss reconciliation; the latter covers direct P2P, server-relay fallback, and direct retry after UDP recovery.
